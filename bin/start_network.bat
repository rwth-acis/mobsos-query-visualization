:: this script is autogenerated by 'ant startscripts'
	:: it starts a LAS2peer node providing the service 'i5.las2peer.services.mobsos.queryVisualization.QueryVisualizationService' of this project
	:: pls execute it from the bin folder of your deployment by double-clicking on it

	%~d0
	cd %~p0
	cd ..
	set BASE=%CD%
	set CLASSPATH="%BASE%/lib/*;"

	java -cp %CLASSPATH% i5.las2peer.tools.L2pNodeLauncher -p 9011 uploadStartupDirectory startService('i5.las2peer.services.mobsos.queryVisualization.QueryVisualizationService@0.6.7.1','someNewPass') startWebConnector interactive

	pause
	
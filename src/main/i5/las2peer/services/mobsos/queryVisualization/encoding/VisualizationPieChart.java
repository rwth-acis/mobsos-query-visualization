...
public class VisualizationPieChart extends Visualization {
  ...
  public String generate(
    ...
    try {
      if (methodResult == null) {
        throw new Exception(
          "Tried to transform an invalid (method) result set into Google Piechart Code!"
        );
      }
      ...
      resultHTML.append(
        ...
        randomNodeId +
        "' style='height: " +
        visualizationParameters[1] +
        "; width: " +
        visualizationParameters[2] +
        ";'></div>\n"
      );
      resultHTML.append("<script>\n");
      resultHTML.append("var qv_script = document.createElement('script');\n");
      resultHTML.append(
        "qv_script.src = 'https://www.google.com/jsapi?callback=qv_loadChart';\n"
      );
      resultHTML.append("qv_script.type = 'text/javascript';\n");
      resultHTML.append(
        "document.getElementsByTagName('head')[0].appendChild(qv_script);\n"
      );
      resultHTML.append("function qv_loadChart(){\n");
      resultHTML.append(
        "google.load('visualization', '1', {packages: ['corechart'], callback: qv_drawChart});\n"
      );
      resultHTML.append("}\n");
      ...
      if (columnCount != 2) {
        throw new Exception(
          "Cannot draw pie-chart with more than two columns as input!"
        );
      }

      resultHTML
        .append("data.addColumn('string', '")
        .append(columnNames[0])
        .append("');\n");
      resultHTML
        .append("data.addColumn('number', '")
      ...
      try {
        return super.visualizationException.generate(
          e,
          "Encoding into Pie Chart failed."
        );
      } catch (Exception ex) {
        Context
          .get()
  ...
  private String getRandomId(int length, boolean startWithLetter) {
    ...
    for (int i = 0; i < length; i++) {
      text +=
        possible.charAt((int) Math.floor(Math.random() * possible.length()));
    }

...
public class QueryVisualizationService extends RESTService {
	...
	private MethodResult transformToMethodResult(JSONObject json) throws ServiceException {
		try {
			MethodResult methodResult = new MethodResult();
			JSONArray data = null;
	...
	private byte[] visualizeQueryImage(Query query, String[] queryParameters, String format) throws Exception {
		MethodResult methodResult = null;
		Connection c = null;
		if (query.usesCache() && queryParameters == null) {
			methodResult = resultCache.get(query.getKey());
		...
		if (visualization.check(methodResult, query.getVisualizationParameters())) {
			Context.get().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_16, "" + query, true);
			byte[] v = visualization.generatePNG(methodResult, query.getVisualizationParameters());
			c.close();
			return v;
		} else {
			c.close();
	 ...
	 * @throws Exception
	 */
	public void addDatabase(String databaseKey, Integer databaseTypeCode, String username, String password,
			String database, String host, Integer port) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void addDatabase(String databaseKey, SQLDatabaseType databaseTypeCode, String username, String password,
			String database, String host, Integer port) throws Exception {
	...
	public static class Resource {
		...
		@Consumes(MediaType.APPLICATION_JSON)
		@ApiOperation(
				value = "Returns the chosen visualization.")
		@ApiResponses(
				value = { @ApiResponse(
						code = 200,
						message = "Created query.",
						...
						@ApiResponse(
								code = 400,
								message = "Creating Query failed."), })
		public Response visualize(@ApiParam(
				value = "Visualization type.") @QueryParam("format") @DefaultValue("GOOGLEPIECHART") String vtype,
				...
				@QueryParam("height") @DefaultValue("200") String height, @ApiParam(
						value = "Query information.",
						required = true) JSONObject json) {
			try {
				...
				Visualization visualization = service.visualizationManager.getVisualization(v);
				...
				Context.get().monitorEvent(this, MonitoringEvent.SERVICE_CUSTOM_MESSAGE_10, "" + vtype, true);
				if (output.equals("png")) {

					byte[] imageData = visualization.generatePNG(methodResult, visualizationParamters);
					...
					String htmlString = visualization.generate(methodResult, visualizationParamters);
		...
		public Response visualizeQueryByKey(@ApiParam(
			...
			try {
				...
				if (format.equals("PNG")) {
					byte[] imageData = service.visualizeQueryImage(query, queryParameters, format.toUpperCase());
					ResponseBuilder responseBuilder = Response.ok(imageData);
					responseBuilder.header(HttpHeaders.CONTENT_TYPE, "image/png");
					return responseBuilder.build();
				} else {
